...
optionParser hereAndNow = Options
    <$> argument str
        (  metavar "FILE"
        <> value defaultFile )
    <*> option (fmap Just . dateTimeReader hereAndNow =<< str)
        (  long "start"
        <> short 's'
        <> metavar "TIME"
...
dateTimeReader hereAndNow input = result $ listToMaybe $ catMaybes parses where
    ...
    byFormat format = maybe Nothing toLocalZone $ parseTimeM True locale format input
...
parsePing here line = do
    ...
    time <- parseTimeM True locale "%s" $ Text.unpack timestr
...
printLevelGraph pings = do
    ...
    let content = [ ("Work", unitWeight work)
